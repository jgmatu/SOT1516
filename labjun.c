...
changeext(char *test , char *ext , char *newext)
{
	...
	auxext = strncpy(newtest + strlen(newtest) , newext , strlen(newtest));
...
change_env (char **parameters , int found)
{
	...
	if (getenv(env) != NULL)  {
		...
		if (parameters[found] == NULL)
			err (1 , "Error strdup to change the name of envoirment \
					variable : %s" , strerror(errno));
	} else {
		fprintf(stderr, "%s %s\n", "Envoirment name of variable not \
		 			found : " , env);
		exit(1);
	}
}
...
exec_cmd (char *cmd , char **parameters)
{
	...
	for (i = 0 ; i < numpath ; i++){
		...
		strncat(cmdpath , path[i] , strlen(cmdpath) + strlen(path[i]));

		if (!endpath(cmdpath))
			strcat(cmdpath , "/");

		strncat(cmdpath , cmd , strlen(cmdpath) + strlen(path[i]));
...
launchtest (char *test , struct T_FILES *files , int timeout)
{
	...
	case  0:
		...
		if (getenv("CEXT"))
			testout = changeext(test , getenv("CEXT") , EXTOUT);
		else
...
is_ext_basic_test (char *name)
{
	...
	if (getenv("CEXT") != NULL)
		ext = strdup(getenv("CEXT"));

	if (ext == NULL)
		err (1 , "Error in change extension %s" , strerror(errno));

	return (isext(name , ext) || isext(name ,  EXTOK));
}
