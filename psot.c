...
cmd_line_pipe (struct T_CMDPIPE *cmdpipe)
{
	...
	case  0:
		...
		if (stdin_to_fd(cmdpipe->readLastPipe) < 0)
		  /* Standard input to next-to-last pipe created we have to read last output of that pipe in this process. */
			exit(1);
		...
		if (close(fdpipe[0]) < 0)
		  /* Close read pipe in this process necessary. Close test.tst is important too.*/
...
checkok (struct T_FILES *files , char *testout)
{
	...
	testok = changeext(testout , EXTOUT , EXTOK);
...
static void
freefiles (char **files , int numfiles)
{
	int i;
...
cunit_basic_test(char *path , int timeout)
{
	...
	freefiles(files->files , files->numfiles);
	free(pidstests);
	free(files);
	return cunitfailed;
}
